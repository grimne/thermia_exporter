...
func main() {
	...
	intervalStr := envOrDefault("THERMIA_SCRAPE_INTERVAL", "900") // seconds
	...
	gOperationalStatus := prometheus.NewGaugeVec(prometheus.GaugeOpts{Name:"thermia_operational_status_running", Help:"Operational status bits that are running (1)"}, append(labels, "status"))
	...
	go func() {
		for {
			func() {
				...
				// Online + last online
				if sum.Online { gOnline.With(lbls).Set(1) } else { gOnline.With(lbls).Set(0) }
				if ts := sum.LastOnlineUnix; ts > 0 { gLastOnlineUnix.With(lbls).Set(float64(ts)) }

				// Temps
				if v, ok := sum.Temperatures["indoor"]; ok { gIndoor.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["outdoor"]; ok { gOutdoor.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["supply_line"]; ok { gSupply.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["desired_supply_line"]; ok { gDesiredSupply.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["return_line"]; ok { gReturn.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["buffer_tank"]; ok { gBuffer.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["hot_water"]; ok { gHotWater.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["brine_out"]; ok { gBrineOut.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["brine_in"]; ok { gBrineIn.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["pool"]; ok { gPool.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["cooling_tank"]; ok { gCoolTank.With(lbls).Set(v) }
				if v, ok := sum.Temperatures["cooling_supply"]; ok { gCoolSupply.With(lbls).Set(v) }

				// Operation modes
				for _, m := range sum.OperationModesAvailable { gOperationModeAvailable.With(merge(lbls, "mode", m)).Set(1) }
				if sum.OperationMode != "" { gOperationMode.With(merge(lbls, "mode", sum.OperationMode)).Set(1) }

				// Statuses
				for _, s := range sum.OperationalStatusAvailable { gOperationalStatusAvail.With(merge(lbls, "status", s)).Set(1) }
				for _, s := range sum.OperationalStatusRunning { gOperationalStatus.With(merge(lbls, "status", s)).Set(1) }
				for _, s := range sum.PowerStatusAvailable { gPowerStatusAvail.With(merge(lbls, "status", s)).Set(1) }
				for _, s := range sum.PowerStatusRunning { gPowerStatus.With(merge(lbls, "status", s)).Set(1) }

				// Hot water
				if sum.HotWaterSwitch != nil { gHWSwitch.With(lbls).Set(float64(*sum.HotWaterSwitch)) }
				if sum.HotWaterBoost != nil { gHWBoost.With(lbls).Set(float64(*sum.HotWaterBoost)) }

				// Operational time
				if v, ok := sum.OperationalTimeHours["REG_OPER_TIME_COMPRESSOR"]; ok { gOpComp.With(lbls).Set(float64(v)) }
				if v, ok := sum.OperationalTimeHours["REG_OPER_TIME_HEATING"]; ok { gOpHeat.With(lbls).Set(float64(v)) }
				if v, ok := sum.OperationalTimeHours["REG_OPER_TIME_HOT_WATER"]; ok { gOpHW.With(lbls).Set(float64(v)) }
				if v, ok := sum.OperationalTimeHours["REG_OPER_TIME_IMM1"]; ok { gOpImm1.With(lbls).Set(float64(v)) }
				if v, ok := sum.OperationalTimeHours["REG_OPER_TIME_IMM2"]; ok { gOpImm2.With(lbls).Set(float64(v)) }
				if v, ok := sum.OperationalTimeHours["REG_OPER_TIME_IMM3"]; ok { gOpImm3.With(lbls).Set(float64(v)) }
